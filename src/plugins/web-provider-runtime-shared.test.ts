...
describe("web-provider-runtime-shared", () => {
  ...
  it("uses prepared Gateway runtime web providers before runtime plugin loads", () => {
    const activeRegistry = { source: "active" };
    const gatewayRuntimeRegistry = { source: "gateway-runtime" };
    const mapRegistryProviders = vi.fn(({ registry }) =>
      registry === gatewayRuntimeRegistry ? ["provider"] : [],
    );
    mocks.getLoadedRuntimePluginRegistry.mockImplementation((params: unknown) => {
      const surface = (params as { surface?: string } | undefined)?.surface;
      return surface === "gateway-runtime"
        ? (gatewayRuntimeRegistry as never)
